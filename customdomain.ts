// Code obtenu d'un exemple fourni par Pulumi: https://github.com/pulumi/examples/blob/master/azure-ts-dynamicresource/cdnCustomDomain.ts
...
class CDNCustomDomainResourceProvider implements pulumi.dynamic.ResourceProvider {
    ...
    constructor(name: string) {
        ...
        this.clientID = config.require("clientId");
        this.clientSecret = config.require("clientSecret");
        this.tenantID = config.require("tenantId");
        this.subscriptionID = config.require("subscriptionId");
    }

    private async getCDNManagementClient(): Promise<cdnManagement.CdnManagementClient> {
        const credentials = await msRestAzure.loginWithServicePrincipalSecret(this.clientID,
            this.clientSecret, 
            this.tenantID);
        return new cdnManagement.CdnManagementClient(credentials, this.subscriptionID);
    }
